export const IngestionDatasetQuery = {
    async getDataset(datasetName) {
        const queryStr = `SELECT schema FROM spec.DatasetGrammar WHERE c_name = $1`;
        return {query: queryStr, values: [datasetName]};
    },
    async getDimension(dimensionName) {
        const queryStr = `SELECT schema FROM spec.DimensionGrammar WHERE c_name = $1`;
        return {query: queryStr, values: [dimensionName]};
    },
    async getEvents(eventName) {
        const queryStr = `SELECT schema FROM spec.EventGrammar WHERE c_name = $1`;
